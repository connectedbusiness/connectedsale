...
define([
  ...
  'collection/base',
  'text!template/25.0.0/pos/item/header-info/shipto/shiptoform.tpl.html',
  'view/spinner',
  'view/25.0.0/pos/postal/addpostal'
], function($, $$, _, Backbone,
  ...
  var ShipToFormView = Backbone.View.extend({
    ...
    //  this.CheckProductEdition();
      ...
      if (!Global.Preference.AllowChangeClassTemplate) {
        $("#cmb-shipto-classtemplate").attr("disabled", "true");
        $(".shipto2-ClassTemplate-container .ui-icon-arrow-d").addClass("ui-disabled");
      }
    ...
    LoadPostal: function(postal, city) {
      seletedShipToBusinessType = Global.CustomerPreference.DefaultBusinessType;
      if (postal == "") {
        this.ClearCity();
      } else {
        var self = this;
    ...
    SetState: function() { /*jj*/
      ...
      if (_city != "") {
        ...
        var countryCode = _model.get("CountryCode");
        var postalCode = _model.get("PostalCode");
      ...
      tmpModel.url = Global.ServiceUrl + Service.CUSTOMER + Method.GETSHIPTOCLASSTEMPLATE + countryCode + "/" + _city + "/" + postalCode + "/" + seletedShipToBusinessType;
      tmpModel.save(null, {
        success: function(model, response) {
          if (!Global.isBrowserMode) window.plugins.cbNetworkActivity.HideIndicator();
          if (!model.get("ClassCode")) return;
          ...
          if (self.formType == FormType.NewShipTo.FormTitle) {
            ...
            self.paymentTermCode = model.get("PaymentTermCode");
            self.taxCode = model.get("TaxCode");
             $("#cmb-shipto-classtemplate").val(self.classTemplate);
          }
          self.FetchClassCodes(countryCode);
        },
        error: function(model, error, response) {
          if (!Global.isBrowserMode) window.plugins.cbNetworkActivity.HideIndicator();
          //model.RequestError(error, "Error Fetching Class Code");
    ...
    SetShipToDetails: function() {
      ...
      this.taxCode = $("#shipto-TaxCode").val();
      this.paymentTermCode = $("#shipto-PaymentTerm").val();
    ...
    EditShipTo: function(model) {
      ...
      if (Global.DefaultClassCode == null || Global.DefaultClassCode == undefined || Global.DefaultClassCode == "") Global.DefaultClassCode = model.get("ClassCode");
      ...
      this.FetchClassCodes(model.get("Country"));
    },
    ...
    ValidateFields: function() {
     var postal = $("#shipto-PostalCode").val();
      ...
      //   //console.log("email");
      // } 
      else if ((email != null && email != "") && this.ValidateEmailFormat(email) && !isAllowByPassValidation) {
