...
define([
  ...
  var SummaryItem = BaseModel.extend({
    defaults: {
      ...
      "TermDiscount": 0.00,
      "TotalDiscount": 0.00,
      "TotalCreditMemoAmount": 0.00
    },

    Balance: function() {
      //  var _tax = parseFloat(this.RoundNumber(format("0.00000", this.get("TotalTax")), 4)),
       var _tax = format("0.0000", this.get("TotalTax").toFixed(4)),
    ...
    BalanceWithoutPayment: function() {
      //  var _tax = parseFloat(this.RoundNumber(format("0.00000", this.get("TotalTax")), 4)),
       var _tax = format("0.0000", this.get("TotalTax").toFixed(4)),
      _bal = parseFloat(this.get("SubTotal").toFixed(2)) + parseFloat(this.RoundNumber(_tax, 2));
