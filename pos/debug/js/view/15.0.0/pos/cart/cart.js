...
define([
  ...
  'shared/enum',
  'shared/shared',
  'view/15.0.0/pos/cart/total',
  'view/15.0.0/pos/cart/cartItem',
  'view/15.0.0/pos/cart/kit',
  'view/15.0.0/pos/cart/summary',
  'view/15.0.0/pos/itemdetail/itemdetail',
  'view/15.0.0/pos/seriallot/seriallot',
  'model/summary',
  'text!template/15.0.0/pos/cart/cart.tpl.html',
  'js/libs/iscroll.js',
  'js/libs/format.min.js'
], function($, $$, _, Backbone, BigDecimal, Global, Enum, Shared,
  TotalView, CartItemView, KitView, SummaryView, ItemDetailView, SerialLotView, SummaryModel, template) {
  var CartView = Backbone.View.extend({
    ...
    AddNetToInvoiceTotal: function(item, type) {
      ...
      var _taxDisplay = parseFloat(this.RoundNumber(format("0.00000", _tax), 2));

      if (this.IsReturn()) {
        var _rnd = parseFloat(parseFloat(_tax).toFixed(4)),
          _trim = parseFloat(this.preciseRound(_tax, 2));

        if (parseFloat((_rnd - _trim).toFixed(3)) == 0.005) _taxDisplay = _trim;
        else _taxDisplay = _rnd;
        _taxDisplay = format("#,##0.00", this.RoundNumber(format("0.00000", _taxDisplay), 2));
      }
