...
define([
  ...
  'view/15.0.1/pos/actions/actions',
  'view/15.0.1/pos/item/header-info/headerinfo',
  'view/15.0.1/pos/item/items',
  'view/15.0.1/pos/cart/cart',
  'view/15.0.1/pos/item/category/categories',
  'view/15.0.1/pos/payment/payment',
  'view/15.0.1/pos/tax/taxlist',
  'view/15.0.1/pos/item/search/product/products',
  'view/15.0.1/pos/item/search/productdetail',
  'view/15.0.1/pos/item/search/stock/stocks',
  'view/15.0.1/pos/transactiontype/transactiontype',
  'view/15.0.1/pos/transactions/transactions',
  'view/15.0.1/pos/payment/payments',
  'view/15.0.1/pos/coupon/coupon',
  'view/15.0.1/pos/signature/signature',
  'view/15.0.1/pos/reason/transactionreason',
  'view/15.0.1/pos/print/printoptions',
  'view/15.0.1/pos/print/printpreview',
  'view/15.0.1/pos/print/dynamicprint',
  'view/15.0.1/pos/print/printer',
  'view/15.0.1/pos/manageroverride/manageroverride',
  'view/15.0.1/pos/payment/refund',
  'view/15.0.1/pos/drawerbalance/openingamount',
  'view/15.0.1/pos/drawerbalance/closingamount',
  'view/15.0.1/pos/report/statusreport',
  'view/15.0.1/pos/payment/paymenttype',
  'view/15.0.1/pos/seriallot/seriallot',
  'view/15.0.1/pos/notes/notescontrol',
  'view/15.0.1/pos/customerpo/customerpo',
  'view/15.0.1/pos/giftcard/giftcard',
  'view/15.0.1/pos/item/upcitem/upcitems',
  'view/15.0.1/pos/item/gcitem/gcitems',
  'view/15.0.1/pos/pickup/pickuplist',
  'view/15.0.1/pos/bundle/configurator',
  'view/15.0.1/pos/kit/configurator',
  'text!template/15.0.1/header/header.tpl.html',
  'text!template/15.0.1/pos/item/search/search.tpl.html',
  'text!template/15.0.1/pos/item/search/lookup.tpl.html',
  'text!template/15.0.1/pos/pos.tpl.html',
  'view/spinner',
  'js/libs/iscroll.js',
  ...
  CustomerPOView, GiftCardView, UPCItemsView, GCItemListView, PickupListView, BundleConfiguratorView, KitConfiguratorView,
...
//      }
        if(hide) {
          ...
          this.$('.toolbar-header-right').attr('style','width: 18.9%;');
          $(".label-transactioncode").css("width", "115%");
        }
    ...
    **/
    ...
    AddSelectedUpcItem: function(collection) {
      ...
      collection.each(function(upcitem) {
        ...
        var existingCartItem = self.cartCollection.find(function(cartItem) {
          ...
          if (isReturn) return cartItem.get("ItemCode") === itemCode && cartItem.get("UnitMeasureCode") === umCode && cartItem.get("IsNewLine");
    ...
    AddToCart: function(newItem) {
      ...
      var _itemCode = newItem.get("ItemCode"),
        ...
        _existingCartItem = this.cartCollection.find(function(cartItem) {
          return _isReturn ? (cartItem.get('ItemCode') == _itemCode && cartItem.get('UnitMeasureCode') == _unitMeasureCode && cartItem.get('IsNewLine') && cartItem.get('ItemType') != Enum.ItemType.Kit) :
            (cartItem.get('ItemCode') == _itemCode && cartItem.get('UnitMeasureCode') == _unitMeasureCode && cartItem.get('ItemType') != Enum.ItemType.Kit);
        });
    ...
    NotifyIfItemIsOutOfStock: function(itemAttr, forceCheck) {
      ...
      var _msg1 = "Item '" + itemAttr.ItemName + "' does not have enough stock. ConnectedSale will automatically adjust stock when the sale transaction is created.";
      var _msg2 = "However, this requires manager's approval.";
      var _msgTitle = "Stock Verification";
      ...
      if (!Global.Preference.AutoAllocateOverrideLevel ||
        ...
        navigator.notification.alert(_msg1, null, _msgTitle, "OK");
        return;
      }
    ...
    /**
		 Validate Transaction, manager override, reasoncodes and Prompt To Printbefore creating Credit Memo
    ...
    AddRefund: function() {
      ...
      switch (Global.PaymentType) {
        case Enum.PaymentType.Cash:
          this.refundView.AddCashPayment(this.GetRefundAmount());
          break;
        default:
    ...
    CreateInvoice: function(IsPosted) {
      ...
      this.transactionModel.url = Global.ServiceUrl + Service.SOP + Method.CREATEINVOICE + isCreateCustomer;
    ...
    CreateOrder: function() {
      ...
      var isCreateCustomer = "false";
      this.transactionModel.url = Global.ServiceUrl + Service.SOP + Method.CREATESALESORDER + isCreateCustomer;
      //use save method of model to save data to the server then alert the created invoice code
    ...
    CreateQuote: function() {
      ...
      this.transactionModel.url = Global.ServiceUrl + Service.SOP + Method.CREATEQUOTE;
    ...
    UpdateQuote: function(salesOrderCode) {
      ...
      this.RemoveWarehouseCodeByItemType(this.transactionModel.get("SalesOrderDetails"));
      this.transactionModel.url = Global.ServiceUrl + Service.SOP + Method.UPDATEQUOTE;
...
//          if (invoice.SurplusMessage) navigator.notification.alert(invoice.SurplusMessage, null, "Surplus Amount", "OK", true);
    ...
    SetTransactionCodeDisplay: function(transactionCode) {
      if (transactionCode) {
        ...
        $(".label-transactioncode").css("font-size", "18px");
        $(".arrow-down").css("opacity", "0.3");
      } else {
    ...
    DoLoadOrder: function(salesOrderCode, type, isChangeWarehouse) {
      ...
      this.orderTransactionModel.url = Global.ServiceUrl + Service.SOP + Method.LOADORDER + _type;
      this.LockTransactionScreen(true, "Loading...");
    ...
    ConvertQuote: function(salesOrderCode) { //jjx
      ...
      this.transactionModel.url = Global.ServiceUrl + Service.SOP + Method.CONVERTQUOTETOSALESORDER + salesOrderCode;
      this.LockTransactionScreen(true, "Saving...");
      Global.SVG_ReadyToSave = true;
      this.ResetCustomerPODetails();
      ExecThis();
      //var _transactionModel = this.transactionModel;
    },
    ...
    RecalculateCouponCompleted: function(couponGroup, isAcceptCoupon) {
      ...
      if (coupon) {
        ...
        this.ChangeItemGroupTax(this.cartCollection, isAcceptCoupon);
      }
    ...
    GetCustomerPaymentByTransactionCode: function(transactionCode, transactionModel) {
      ...
      switch (transactionModel.get("TransactionType")) {
        ...
        case Enum.TransactionType.Order:
          ...
          tempModel.url = Global.ServiceUrl + Service.SOP + Method.LOADORDER + Enum.TransactionType.Order;
          break;
    ...
    GetReportCode: function(type, preference) {
      ...
      if (type == "CreateCreditMemo" || type == "ConvertInvoice" || type == "CreateRefund") {
        ...
        if (!Global.Preference.IsAirprint) {
          ...
          reportCode = preference.InvoiceReportCode;
        }
      } else if (type == "CreateOrder" || type == "UpdateOrder" || type == "ConvertQuote") {
        if (!Global.Preference.IsAirprint) {
      ...
      if (!Shared.IsNullOrWhiteSpace(this.paymentCollection) || !Shared.IsNullOrWhiteSpace(this.customerPaymentCollection)) {
        if (!Shared.IsNullOrWhiteSpace(this.paymentCollection) && this.paymentCollection.length > 0) {
          ...
          if (_hasCreditCardPayment) {
            _creditCardReportCode = Global.Preference.CreditCardReportCode;
          }
        }
    ...
    TaxChangeSaveSuccess: function(model, response, options) {
      ...
      this.NotifyIfItemIsOutOfStock(this.item.attributes);
